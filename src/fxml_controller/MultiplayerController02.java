...
/**
 * @author D067928
 * Die Klasse zust�ndig f�r den Spielstart im Mehrspieler. Fragt vorher die Namen ab.
 */
public class MultiplayerController02 implements Initializable {
	...
	//setzt vorl�ufig die Spieleranzahl auf den Gamemode
	...
	/**
	 * Wenn ein Name eingegeben wird, wird Checknames ausgef�hrt, welche pr�ft ob die Namen nicht leer sind.
	 */
	@FXML
	...
	private void init_game() {
		//Zun�chst wird der Ladebalken initialisiert.
		ProgressBar progressBar = new ProgressBar(0);
		progressBar.setPrefSize(400, 40);
		//Es wird ein neuer Service erstellt, welcher daf�r sorgt dass JavaFX die Berechnung nicht im UI-Thread �bernimmt
		//Ansonsten w�rde das UI bis zum fertiggeladenen Screen nicht mehr reagieren.
		Service<Void> sv = new Service<Void>() {
			@Override
			protected Task<Void> createTask() {
				//Dies l�dt alle Bilder
				return IMGhandler.initialize(Start.getJhdl().getModel().getInfo().getCardcount());
			}
		};
		//es wird ein Echtzeitupdater initialisiert, der regelm��ig den Fortschritt im Ladebalken aktualisiert und bei fertigem
	...
	public void initialize(URL location, ResourceBundle resources) {
		//Falls sich der Gamemode �ndert werden auch die Textfelder refreshed (andere Spieleranzahl)
